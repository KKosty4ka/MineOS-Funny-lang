...
local function gpuSetBackground( ... )
	gpu.setBackground( math.random(0x000000, 0xFFFFFF) )
end

local function gpuSetForeground( ... )
	gpu.setForeground( math.random(0x000000, 0xFFFFFF) )
end
	...
	function(proxy)
		for i = 1, #OSList do
			if proxy.exists(OSList[i][1]) then
				status(stringsMain, "Бутаемся с " .. (proxy.getLabel() or proxy.address))
...
while uptime() < deadline do
	...
	if eventData[1] == stringKeyDown and eventData[4] == 56 then
		local utilities = {
			menuElement("Жопа мира", function()
					...
					for address in cl(stringsFilesystem) do
						...
						if not isReadOnly then
							tableInsert(filesystemOptions, menuElement(stringsChangeLabel, function()
								proxy.setLabel(input(title(2, stringsChangeLabel), "Кукиш или фига: "))
								updateFilesystems()
							end, 1))
						...
						tableInsert(filesystems, 1,
							menuElement(
								...
								restrict(label, 12) ..
								restrict(proxy.spaceTotal() > 1048576 and "ХДД" or proxy.spaceTotal() > 65536 and "ФДД" or "СИС", 3) ..
				...
				updateFilesystems()
				menu("Выбирете жопу", filesystems)
			end),
		...
		if internetAddress then	
			tableInsert(utilities, 2, menuElement("Интернет рековери", function()
				...
				if handle then
					...
					status(stringsMain, "жопа кому!", 1)
				end
			end))
		end
...
if not (proxy and boot(proxy)) then
	...
	if not proxy then
		status(stringsMain, "Компуктер СДОХ!", 1)
	end
end
