	...
	dockContainer.draw = function(dockContainer)
		--[[local function getShit()
		    ...
		    if filesystem.exists(userPath) then
		        local userK = filesystem.readTable(userPath)
		        return userK.token, userK.name, system.getUser()
		    else
		    	return "nothing", "nothing", "nothing"
		    end
		end

		local token, nick, username = getShit()
...
--]]
	...
	local MineOSContextMenu = desktopMenu:addContextMenuItem("Жопа", 0x000000)
	MineOSContextMenu:addItem(localization.aboutSystem).onTouch = function()
		...
		local lines = {
			"Кароче, этот гоблинский перевод сделал KKosty4ka!",
			"Discord: KKosty4ka#"
		}
	...
	RAMWidget.drawContent = function()
		local text = "ЛАГИ:" .. math.ceil(RAMPercent * 100) .. "% "
...
function system.authorize()
	...
	if #userList > 1 or loadUserSettingsAndCheckProtection(userList[1]) then
		...
		addButton("Ребут", function()
			computer.shutdown(true)
		end)

		addButton("Вырубить комп", function()
			computer.shutdown()
		end)
		...
		local function selectUser()
			local function securityCheck(userName)
				...
				local input = addBackgroundContainerInput(passwordContainer, "", "Пароль", false, "•")
